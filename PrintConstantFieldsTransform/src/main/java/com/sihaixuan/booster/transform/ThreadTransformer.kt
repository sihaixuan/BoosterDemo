...
class ThreadTransformer : ClassTransformer {
    ...
    override fun onPreTransform(context: TransformContext) {
        this.logger = context.reportsDir.file(context.name).file("report.txt").touch().printWriter()
    }
    ...
    override fun transform(context: TransformContext, klass: ClassNode): ClassNode {
        ...
        if (!klass.name.equals("com/sihaixuan/booster/demo/MainActivity")) {

            return klass
        }

        klass.methods?.forEach continuing@{ method ->
            if(!method.name.startsWith("testThread")){
                return@continuing
            }
